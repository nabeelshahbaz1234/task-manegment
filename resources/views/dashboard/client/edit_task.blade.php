...
{{-- <link rel="stylesheet" href="{{asset('css/toastr1.css')}}" >
<link rel="stylesheet" href="{{asset('css/toastr2.css')}}" > --}}
...
<form action="{{route('client.update_task',$task->id)}}" method="post" enctype="multipart/form-data">
    <input type="hidden" name="edit_id"  >
    <div class="form-group">
        @csrf
        <label> Title: </label>
        <input type="text" name="title" value="{{$task->title}}" class="form-control" placeholder="Enter Task title">
    </div>
               {{-- @dd($pros) --}}
    <div class="form-group">
...
<div class="img-wrap">
     ...
     @foreach (json_decode($task->Task_images) as $image)
      ...
      <a href="{{route('client.delete_image',$image->id)}}">
